...
def main(args: dict[str: any]):
  logger = recognizer.logger.Logger(identity="Recognizer", debugEnabled=args["debug"]) # Setups Logger
  ...
  detector = cv2.CascadeClassifier(args["cascade"]) # Detector to use, Path grabbed from program arguments
  font = cv2.FONT_HERSHEY_SIMPLEX # Font to use

  id = 0 # Id of Target

  names = ['None', 'Simon'] # Default Trained Targets
  ...
  while True:
    ...
    faces = detector.detectMultiScale(image=gray, scaleFactor=1.3, minNeighbors=5, minSize=(int(minWidth), int(minHeight)),)
    ...
    for(xPos, yPos, width, height) in faces:
      ...
      cv2.rectangle(img=img, pt1=(xPos, yPos), pt2=(xPos + width, yPos + height), color=(args["recogRectRed"], args["recogRectGreen"], args["recogRectBlue"]), thickness=2)

      # Asks the trained Recognizer to attempt to predict the target as well as the confidence of it's prediction
      id, confidence = recog.predict(gray[yPos : yPos + height, xPos : xPos + width])
      ...
      if (confidence < 100):
          id = names[id] # Grabs the name based off ID from the predefined list
          confidence = f"  {round(100 - confidence)}%" # Turns the confidence value into a % string value
      else: # If unable to recognize the target
          id = "unknown" # Name becomes unknown
          confidence = f"  {round(100 - confidence)}%" # Confidence on it being a face

      # Writes the recognized name
      cv2.putText(img, str(id), (xPos + 5, yPos - 5), font, 1, (int(args["recogTextBlue"]), int(args["recogTextGreen"]), int(args["recogTextRed"])), 2)
      # Writes the confidence value next to said name.
      cv2.putText(img, str(confidence), (xPos + 5, yPos + height - 5), font, 1, (int(args["recogTextConfBlue"]), int(args["recogTextConfGreen"]), int(args["recogTextConfRed"])), 1)  
