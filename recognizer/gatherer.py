...
def main(args: dict[str: any], isCamera: bool):
    ...
    if isCamera: 
        ...
        logger.info(input="Please enter the amount of pictures to take")
        amount = input()
        while not face_off.is_int(amount):
            logger.warn("Error: Invalid Input")
            logger.warn("Please enter the amount of pictures to take")
            amount = input()
        amount = int(amount)
        ...
        while(True):
            ...
            for (xPos, yPos, width, height) in faces:
                ...
                path = args["dspath"] + str(face_id)
                if not os.path.exists(path):
                    os.mkdir(path)
                with open(file=args["dspath"] + str(face_id) + "/User." + str(face_id) + "." + str(count) + ".jpg", mode='w', encoding='utf-8'):
                    pass
                cv2.imwrite(filename=args["dspath"] + str(face_id) + "/User." + str(face_id) + "." + str(count) + ".jpg", img=gray[yPos: yPos + height, xPos: xPos + width])
                ...
                k = cv2.waitKey(delay=1000) & 0xff  # Press 'ESC' for exiting video
        ...
        search_target = queryAndGetInput(logger, "Please specify a target for the image search: ")
        search_amount = queryAndGetInput(logger, "Please enter the amount of images you want to download: ")

        while not face_off.is_int(search_amount):
            logger.warn("Error: Invalid Input")
            search_amount = queryAndGetInput(logger, "Please enter the amount of images you want to download: ")
        search_amount = int(search_amount)

        save_path = queryAndGetInput(logger, "What should the save sub-folder be called?: ")
        path = args["dspath"] + save_path
        if not os.path.exists(path):
            os.mkdir(path)
        search = URL_PREFIX + search_target + URL_POSTFIX
        ...
        elem1 = driver.find_elements(By.ID, 'islmp')
        sub = []
        for elem in elem1:
            sub.append(elem.find_elements(By.TAG_NAME, 'img'))  

        subTagIndex = 0
        for elem in sub:
            for subTagIndex, i in enumerate(elem):
                time.sleep(0.5)
                if subTagIndex <= search_amount:
                    src = i.get_attribute('src')
                    try:
                        if src is not None:
                            src = str(src)
                            cleaned_name = search_target.replace(' ', '_').lower()
                            urllib.request.urlretrieve(src, os.path.join(path, cleaned_name + "_" + str(subTagIndex) + '.jpg'))
                        else:
                            raise TypeError
                    except Exception as ex:       # catches type error along with other errors
                        logger.warn(f'fail with error {ex}')
                else:
                    break
        driver.close()
...
def queryAndGetInput(logger: recognizer.logger.Logger, query: str):
    logger.info(query)
    return input()